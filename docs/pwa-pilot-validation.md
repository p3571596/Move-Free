...
## Recovered engine and revised product direction — 2026-09-20

The original Clinical Decision Engine v0.1 has been recovered and preserved, with a direct rule adapter and parity tests. See `docs/clinical-engine/README.md` for provenance, exact boundaries, input mapping, and deliberate missing-data changes. Patient Today is goal-focused, sessions proceed one exercise at a time, patient navigation is Today / Program / Progress / Messages, and clinician Today is a concise review inbox.

The user explicitly chose to keep the live schema unchanged. Therefore the additive migration is **not applied**. Engine comparison saving, authored conversation replies/history, program-change audit, and founder engine metrics are **Preview**, not live functionality. The recovered engine can be inspected/evaluated with explicit clinician inputs, but no evaluation is claimed saved. Existing manual decisions, patient feedback, current approved guidance, and program updates continue using the existing database.

Isolated Postgres storage tests use PGlite 0.5.8 and synthetic tables representing the required existing schema. Run `tests/clinical-storage.mjs` with `PGLITE_MODULE` pointing to an installed PGlite module. They check ownership, patient/unrelated-clinician/admin exclusion, immutable evaluations, atomic rollback, aggregate founder data, subsequent response association, guidance history, message authorship, program-change capture, and stale-version protection. These are not a substitute for deployed Supabase integration tests after an approved migration.

The current live policies retain pre-existing administrator access exceptions on older clinical tables. This is a **production rollout blocker** for the strict universal care-relationship rule. New prepared tables do not inherit that exception. No live policies were altered or weakened.
