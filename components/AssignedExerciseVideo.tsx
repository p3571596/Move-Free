"use client";
import { PrescriptionSummary } from "./PrescriptionFields";
...
export function AssignedExerciseVideo({ item }: { item: HomeProgramExercise }) {
  ...
  if (item.prescription)
    return (
      <section className="form">
        ...
        <PrescriptionSummary value={item.prescription} />
      </section>
    );
  ...
  if (error)
    return (
      <p role="alert">
        Your demonstration could not be loaded. Reconnect and reload this page
        before starting.
      </p>
    );
  if (asset)
    return (
      <section className="form">
        <p className="eyebrow">Your therapist’s approved demonstration</p>
        <h5>{asset.title}</h5>
        <PrivateVideoPlayer
          key={asset.id}
          path={asset.object_path}
          title={asset.title}
        />
        <p style={{ whiteSpace: "pre-wrap" }}>{asset.instructions}</p>
        {asset.cues ? (
          <p style={{ whiteSpace: "pre-wrap" }}>
            <strong>Key cues:</strong> {asset.cues}
          </p>
        ) : null}
      </section>
    );
  return (
    <>
      ...
      {item.exercise?.patient_instructions ? (
        <p>{item.exercise.patient_instructions}</p>
      ) : null}
      {item.notes ? (
        <p className="therapist-note">
          <strong>Key cues:</strong> {item.notes}
        </p>
      ) : null}
    </>
  );
