...
import { Activity, HeartPulse, Home, LogOut, TrendingUp } from "lucide-react";
...
export function PatientShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="patient-shell">
    <header className="patient-nav"><Link href="/patient" className="brand"><span className="brand-mark">MF</span><strong>Move Free</strong></Link><nav aria-label="Patient navigation"><Link href="/patient"><Home size={17}/>Home</Link><Link href="/patient/program"><Activity size={17}/>Program</Link><Link href="/patient/pain-pattern"><HeartPulse size={17}/>Pain Pattern</Link><Link href="/patient/progress"><TrendingUp size={17}/>Progress</Link><button type="button" onClick={signOut}><LogOut size={17}/>Sign out</button></nav></header>
