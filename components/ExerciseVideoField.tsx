...
export function ExerciseVideoField({initialUrl="",name="Exercise",inputId="video-url",fieldName="video_url",approvalName="video_approved"}:{initialUrl?:string|null;name?:string;inputId?:string;fieldName?:string;approvalName?:string}){
  ...
  return <section className="form" aria-label="Exercise demonstration settings">
    <div className="field"><label htmlFor={inputId}>Video URL (optional)</label><input id={inputId} name={fieldName} type="url" value={url} placeholder="https://www.youtube.com/watch?v=…" onChange={e=>{setUrl(e.target.value);setPreviewed(false);setApproved(false);}}/><p className="muted">Choose a YouTube or Vimeo demonstration you have reviewed. Changes to this library exercise also affect programs that use it.</p></div>
    ...
    <input type="hidden" name={approvalName} value={approved?"yes":""}/>
  </section>;
}
