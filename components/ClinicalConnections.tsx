...
export function ClinicalConnections({ mode }: { mode: "messages" | "team" | "analytics" }) {
  ...
  return <AppShell><RequireAuth><RoleGate allowed={["clinician", "admin"]}>
    ...
    {mode === "messages" ? <div className="panel"><strong>Program-linked communication</strong><p>Patient check-in comments and clinician-approved program guidance are available now. Chat threads, delivery receipts, and push notifications are not available yet.</p></div> : null}
    ...
    <div className="grid" style={{ marginTop: 20 }}>{data?.patients.map(patient => {
      ...
      return <section className="panel" key={patient.id}><h3>{patient.display_name ?? patient.full_name ?? "Patient"}</h3>
        ...
        {mode === "messages" ? <><p>{checkin?.patient_comment ?? log?.notes ?? "No patient comments yet."}</p><Link className="secondary-button" href={`/patients/${patient.id}`}>Review feedback</Link> <Link className="button" href={`/patients/${patient.id}/decision`}>Write guidance</Link></> : <Link className="secondary-button" href={`/patients/${patient.id}${mode === "analytics" ? "/progress" : ""}`}>{mode === "analytics" ? "View clinical outcomes" : "Open patient workspace"}</Link>}
      </section>;
    })}</div>
