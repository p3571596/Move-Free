...
const {saveProgramDraft,loadProgramExercises}=load('lib/data.ts',{'./exercise-media':load('lib/exercise-media.ts'),'./prescription':load('lib/prescription.ts')});
function fixture(){
 ...
 const client={async rpc(name,args){
  assert.equal(name,'stage2_save_program');
  if(tables.home_program_exercises.some(x=>!args.p_items.some(i=>i.id===x.id)))return {error:{message:'Program contains other exercises. Reload before saving.'}};
  for(const [index,item] of args.p_items.entries()){
   const row=tables.home_program_exercises.find(x=>x.id===item.id);
   const value={prescription:item.content,dosage_sets:item.content.sets,dosage_reps:item.content.reps,sort_order:index};
   if(row)Object.assign(row,value);else tables.home_program_exercises.push({id:'new-'+(++next),...value,home_program_id:args.p_program,exercise_id:item.template_id,exercise});
  }
  return {error:null};
 },auth:{getUser:async()=>({data:{user:{id:'clinician'}},error:null})},from(table){let action='select',value,filters=[],order;const q={select(){return q},eq(k,v){filters.push(r=>r[k]===v);return q},in(k,vs){filters.push(r=>vs.includes(r[k]));return q},order(k){order=k;return q},limit(){return q},update(v){action='update';value=v;return q},insert(v){action='insert';value=v;return q},delete(){action='delete';return q},single:async()=>{const r=await run();return {...r,data:r.data[0]}},maybeSingle:async()=>{const r=await run();return {...r,data:r.data[0]??null}},then(resolve,reject){return run().then(resolve,reject)}};
...
test('saving dosage and adding an exercise preserves approved recording assignment and reload order',async()=>{
 ...
 const saved=await saveProgramDraft(f.client,'patient',[{id:'draft-new',exercise_id:f.exercise.id,exercise:f.exercise,sets:1}, {...original,dosage_sets:'3'},f.tables.home_program_exercises[1]]);
 ...
 assert.deepEqual(f.writes.filter(x=>x.table==='home_program_exercises'&&x.action==='delete').flatMap(x=>x.ids),[]);
});
test('removing an exercise with a recording fails before assignment changes or cascade deletion',async()=>{
 const f=fixture();await assert.rejects(saveProgramDraft(f.client,'patient',[f.tables.home_program_exercises[1]]),/Reload before saving/);
