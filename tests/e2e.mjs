...
const messageText='SYNTHETIC message continuity '+Date.now(),replyText='SYNTHETIC reviewed clinician reply '+Date.now();
...
(async()=>{
 ...
 await patient.goto(base+'/patient/messages');await patient.getByText('SYNTHETIC subsequent response after clinician update.',{exact:true}).first().waitFor();await patient.getByText('SYNTHETIC E2E: knee felt harder during the second exercise.',{exact:true}).first().waitFor();pass('Next session response is saved and check-in conversation history survives navigation');
 ...
 await patient.getByLabel('Share an update or question').fill(messageText);await patient.getByRole('button',{name:'Send message',exact:true}).click();await patient.getByText('Message saved.',{exact:true}).waitFor();
 await clinician.goto(base+'/messages/'+fixture.patientId);await clinician.getByText(messageText,{exact:true}).waitFor();
 await clinician.getByLabel('Write a message to your patient').fill(replyText);await clinician.getByRole('button',{name:'Send message',exact:true}).click();await clinician.getByText('Message saved.',{exact:true}).waitFor();
 await patient.getByText(replyText,{exact:true}).waitFor({timeout:45000});await patient.reload();await patient.getByText(messageText,{exact:true}).waitFor();pass('Two-way authored messages persist and refresh across independent sessions');
